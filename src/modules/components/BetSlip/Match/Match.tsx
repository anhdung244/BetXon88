      ...
      {data?.map(({ teamSelected, team1, team2, point, result }, index) => (
        <div className="flex flex-col gap-2 text-[#8a92ab] my-4 border-b border-[#8a92ab] pb-5">
          <Row key={index} justify="space-between">
            <Col span={16} className="flex justify-start items-center gap-2">
              <span>
                <Dot></Dot>
              </span>
              <div className="flex gap-4">
                <span className="font-bold text-gray-300">{teamSelected}</span>
                <span className="font-bold  text-orange-500">{point}</span>
              </div>
            </Col>
